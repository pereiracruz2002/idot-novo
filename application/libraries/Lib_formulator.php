...
class Lib_formulator {
    ...
    function input($type, $args = array()) {
        ...
        if($args['type'] == 'check'){
            $args['extra']['class'] = str_replace('form-control','', $args['extra']['class']);
        }
        ...
        $input = sprintf('<input type="%s" %s/>', $type, (count($_data_extra) ? join(' ', $_data_extra) . ' ' : ''));
        ...
        if (isset($label)) {
            ...
            if (in_array($type, array('checkbox', 'radio')))
                $input = "<label for=\"{$uid}\" class=\"{$label_class} label-{$type}\"> {$input}<span class=\"lbl\"></span> {$label}</label>";
            else
    ...
    function textarea($args = array()) {
        ...
        $textarea = sprintf('<textarea %s>%s</textarea>', (count($_data_extra) ? join(' ', $_data_extra) . ' ' : ''), $value
        );
        if (isset($label)) {
            $classes = '';
            if (isset($class))
    ...
    function select($args = array(), $type = "normal") {
        ...
        $input = sprintf("<select %s>\n%s\n</select>", (count($_data_extra) ? join(' ', $_data_extra) . ' ' : ''), join("\n", $options)
        );
